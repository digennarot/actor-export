...
const fileName = 'StarfinderSecondEdition_CharacterSheet.pdf';
...
/* PAGE 1 */
mapper.textBox('actor name', fileName, 0, 217, 40, 179, 23, character.name, mf_17);
mapper.textBox('player name', fileName, 0, 260, 70, 137, 10, character.ownerName, mf_10);
mapper.textBox('ancestry', fileName, 0, 29, 95, 179, 14, character.ancestry.name, mf_12);
mapper.textBox('heritage', fileName, 0, 29, 123, 147, 13, character.heritage.name, mf_12);
mapper.textBox('size', fileName, 0, 178, 123, 31, 16, character.size, mf_12);

// Background
mapper.textBox('background', fileName, 0, 217, 95, 179, 14, character.background.name, mf_12);
/* TODO: background notes */

// Class
mapper.textBox('class', fileName, 0, 405, 95, 180, 14, character.class.name, mf_12);
mapper.textBox('class notes', fileName, 0, 405, 123, 180, 13, character.class.subClass, mf_12);
...
mapper.textBox('heropoint', fileName, 0, 510, 39, 9, 16, sf2eHelper.evalCheckMark(heroPoints >= 1), mf_17_centered);
mapper.textBox('heropoint', fileName, 0, 533, 39, 9, 16, sf2eHelper.evalCheckMark(heroPoints >= 2), mf_17_centered);
mapper.textBox('heropoint', fileName, 0, 556, 39, 9, 16, sf2eHelper.evalCheckMark(heroPoints >= 3), mf_17_centered);

// Attributes
const attr_x = [29, 123, 217, 311, 405, 499];
Object.values(character.attributes).forEach((a, i) => {
    ref = `${a.name} attribute`;
    mapper.textBox(ref, fileName, 0, attr_x[i], 153, 21, 19, sf2eHelper.quantifyNumber(a.modifier), mf_15_centered);
    ref = `${a.name} attribute boost`;
    const checkMark = sf2eHelper.evalCheckMark(a.isPartialBoost);
    mapper.textBox(ref, fileName, 0, attr_x[i] + 27, 169, 5, 5, checkMark, mf_12_centered);
});

// Defense
mapper.textBox('ac', fileName, 0, 51, 212, 25, 17, character.ac.modifier, mf_17_centered);
mapper.textBox('ac', fileName, 0, 46, 250, 17, 10, character.ac.attributeModifier, mf_10_centered);
mapper.textBox('ac', fileName, 0, 64, 250, 17, 10, character.ac.proficiencyModifier, mf_10_centered);
mapper.textBox('ac', fileName, 0, 82, 250, 18, 10, character.ac.itemModifier, mf_10_centered);

// Shield
if (character.hasShieldEquipped) {
    mapper.textBox('shield', fileName, 0, 104, 207, 15, 17, character.equippedShield.ac, mf_15);
    mapper.textBox('shield', fileName, 0, 127, 211, 21, 10, character.equippedShield.hardness, mf_10_centered);
    mapper.textBox('shield', fileName, 0, 153, 211, 17, 10, character.equippedShield.hpMax, mf_10_centered);
    mapper.textBox('shield', fileName, 0, 170, 211, 18, 10, character.equippedShield.bt, mf_10_centered);
    mapper.textBox('shield', fileName, 0, 192, 211, 16, 10, character.equippedShield.hpValue, mf_10_centered);
}
// Armor proficiencies
...
Object.values(character.defenseProficiencies)
    ...
    .forEach((d, i) => {
        ...
        mapper.textBox(ref, fileName, 0, x, 252, 7, 5, sf2eHelper.evalCheckMark(d.rank >= 2), mf_10_centered);
        mapper.textBox(ref, fileName, 0, x, 259, 7, 5, sf2eHelper.evalCheckMark(d.rank >= 3), mf_10_centered);
        mapper.textBox(ref, fileName, 0, x, 265, 7, 5, sf2eHelper.evalCheckMark(d.rank >= 4), mf_10_centered);
    });
...
Object.values(character.savingThrows).forEach((s, i) => {
    ref = `${s.name} save`;
    const x = saves_x[i];
    mapper.textBox(ref, fileName, 0, x, 206, 38, 18, sf2eHelper.quantifyNumber(s.modifier), mf_15_centered);
    mapper.textBox(ref, fileName, 0, x - 1, 229, 17, 10, s.attributeModifier, mf_10_centered);
    mapper.textBox(ref, fileName, 0, x + 17, 229, 17, 10, s.proficiencyModifier, mf_10_centered);
    mapper.textBox(ref, fileName, 0, x + 35, 229, 17, 10, s.itemModifier, mf_10_centered);

    ref = `${s.name} proficiency`;
    mapper.textBox(ref, fileName, 0, x + 47, 207, 5, 4, sf2eHelper.evalCheckMark(s.rank >= 1), mf_10_centered);
    mapper.textBox(ref, fileName, 0, x + 47, 212, 5, 4, sf2eHelper.evalCheckMark(s.rank >= 2), mf_10_centered);
    mapper.textBox(ref, fileName, 0, x + 47, 217, 5, 4, sf2eHelper.evalCheckMark(s.rank >= 3), mf_10_centered);
    mapper.textBox(ref, fileName, 0, x + 47, 222, 5, 4, sf2eHelper.evalCheckMark(s.rank >= 4), mf_10_centered);
});
...
mapper.textBox('defense notes', fileName, 0, 215, 252, 180, 25, ' '.repeat(28) + defense_notes.join(', '), {
    ...mf_8,
...
mapper.textBox('dying', fileName, 0, 555, 214, 4, 4, sf2eHelper.evalCheckMark(dyingValue >= 1), mf_10_centered);
mapper.textBox('dying', fileName, 0, 564, 214, 4, 4, sf2eHelper.evalCheckMark(dyingValue >= 2), mf_10_centered);
mapper.textBox('dying', fileName, 0, 572, 214, 4, 4, sf2eHelper.evalCheckMark(dyingValue >= 3), mf_10_centered);
mapper.textBox('dying', fileName, 0, 580, 214, 4, 4, sf2eHelper.evalCheckMark(dyingValue >= 4), mf_10_centered);

const wounded = `${character.wounded.value}/${character.wounded.max}`;
mapper.textBox('wounds', fileName, 0, 567, 224, 17, 11, wounded, mf_10_centered);
const resistance_immunities = character.immunities + ', ' + character.resistance;
mapper.textBox('resistance', fileName, 0, 405, 239, 179, 16, resistance_immunities, mf_10);
mapper.textBox('conditions', fileName, 0, 405, 261, 179, 16, character.conditions, mf_10);

/* Skills */
const skill_y = [292, 316, 339, 363, 387, 410, 434, 458, 530, 553, 577, 602, 624, 648, 672, 696, 720, 745];
const lore_skill_y = [484, 508];
...
languages = actor.system.details.languages.value
    ...
    .join(', ');
mapper.textBox('languages', fileName, 0, 236, 295, 108, 74, languages, {
    ...mf_8,
    ...{ valign: 'top', multiline: true },
});
...
mapper.textBox('perception', fileName, 0, 355, 303, 20, 10, perception, mf_12_centered);
mapper.textBox('perception', fileName, 0, 406, 303, 17, 10, character.perception.proficiencyModifier, mf_10_centered);
mapper.textBox('perception', fileName, 0, 388, 303, 17, 10, character.perception.attributeModifier, mf_10_centered);
mapper.textBox('perception', fileName, 0, 424, 303, 17, 10, character.perception.itemModifier, mf_10_centered);
const pRank = character.perception.rank;
mapper.textBox('perception', fileName, 0, 375, 299, 5, 5, sf2eHelper.evalCheckMark(pRank >= 1), mf_8_centered);
mapper.textBox('perception', fileName, 0, 375, 304, 5, 5, sf2eHelper.evalCheckMark(pRank >= 2), mf_8_centered);
mapper.textBox('perception', fileName, 0, 375, 310, 5, 5, sf2eHelper.evalCheckMark(pRank >= 3), mf_8_centered);
mapper.textBox('perception', fileName, 0, 375, 315, 5, 5, sf2eHelper.evalCheckMark(pRank >= 4), mf_8_centered);

mapper.textBox('senses notes', fileName, 0, 355, 328, 110, 41, character.senses, mf_8_multiline);

// speed
mapper.textBox('speed', fileName, 0, 476, 300, 50, 14, character.baseMovement.value, mf_12_centered);

const specialMovement = character.movement
    .filter((f) => !f.isPrimary)
    .map((m) => m.displayName)
    .join(', ');
mapper.textBox('special movement', fileName, 0, 476, 328, 109, 41, specialMovement, {
    ...mf_8,
    ...{ valign: 'top', multiline: true },
});

// Strikes
const melee_strike_y = [410, 460];
const ranged_strike_y = [522, 572, 621];
...
['unarmed', 'simple', 'martial', 'advanced'].forEach((a) => {
    ...
    mapper.textBox('weapon proficiencies', fileName, 0, x, 674, 6, 6, sf2eHelper.evalCheckMark(wp >= 1), mf_8_centered);
    mapper.textBox('weapon proficiencies', fileName, 0, x, 681, 6, 6, sf2eHelper.evalCheckMark(wp >= 2), mf_8_centered);
    mapper.textBox('weapon proficiencies', fileName, 0, x, 688, 6, 6, sf2eHelper.evalCheckMark(wp >= 3), mf_8_centered);
    mapper.textBox('weapon proficiencies', fileName, 0, x, 694, 6, 6, sf2eHelper.evalCheckMark(wp >= 4), mf_8_centered);
});
...
// Class DC
mapper.textBox('class dc', fileName, 0, 237, 715, 70, 20, character.classDC.modifier, mf_15_centered);
mapper.textBox('class dc', fileName, 0, 254, 745, 17, 12, character.classDC.attributeModifier, mf_10_centered);
mapper.textBox('class dc', fileName, 0, 272, 745, 17, 12, character.classDC.proficiencyModifier, mf_10_centered);
mapper.textBox('class dc', fileName, 0, 290, 745, 17, 12, character.classDC.itemModifier, mf_10_centered);
...
mapper.textBox(ref, fileName, 1, 50, 54, 162, 25, ancestryAndHeritageAbilities.join(', '), mf_8_multiline);

// Ancestry Feats
const ancestry_y = [88, 273, 399, 526, 652];
for (let level = 1; level <= 20; level = level + 4) {
    ...
    if (ancestry_y.length > 0) {
        ...
        ancestry_y.shift();
        mapper.textBox('Ancestry feats', fileName, 1, 50, y, 162, 20, feats, mf_8_multiline);
    }
}
...
mapper.textBox(ref, fileName, 1, 50, 124, 162, 43, backgroundSkillFeats.join(', '), mf_8_multiline);

// Skill feats
const skill_feats_y = [178, 241, 304, 367, 430, 493, 556, 619, 682, 744];
for (let level = 2; level <= 20; level = level + 2) {
    ...
    if (skill_feats_y.length > 0) {
        ...
        skill_feats_y.shift();
        mapper.textBox('skill feats', fileName, 1, 50, y, 162, 20, feats.sort().join(', '), mf_8_multiline);
    }
}

// General feats
const general_feats_y = [209, 334, 461, 587, 713];
for (let level = 3; level <= 20; level = level + 4) {
    ...
    if (general_feats_y.length > 0) {
        ...
        mapper.textBox('general feats', fileName, 1, 50, y, 162, 20, feats, mf_8_multiline);
    }
}

// Attribute boosts
const boost_y = [272, 429, 587, 744];
...
// Class Features
const class_features_y = [54, 209, 273, 334, 399, 461, 525, 587, 651, 713];
...
if (character.bonusFeats.length > 0) {
    ...
    mapper.textBox('Bonus Feats', fileName, 1, 216, 109, 179, 58, bonusFeats, mf_8_multiline);
}

// Class Feats
const class_feats_y = [177, 240, 304, 366, 429, 492, 555, 618, 681, 743];
for (let level = 2; level <= 20; level = level + 2) {
...
character.flatItems(character.heldItems).forEach((item) => {
    if (y <= 193) {
        ...
        mapper.textBox(ref, fileName, 1, 406 + xOffset, y, 155 - xOffset, 10, item.displayName, pdfStyle);
        mapper.textBox(ref, fileName, 1, 562, y, 23, 10, item.bulk, mf_8_centered);
        y = y + 10;
    }
});
...
character.flatItems(character.wornItems).forEach((item) => {
    if (y <= 469) {
        ...
        mapper.textBox(ref, fileName, 1, 527, y, 34, 10, sf2eHelper.evalCheckMark(item.isInvested), mf_8_centered);
        mapper.textBox(ref, fileName, 1, 562, y, 23, 10, item.bulk, mf_8_centered);
        y = y + 10;
    }
});
...
// Bulk
mapper.textBox('bulk', fileName, 1, 413, 614, 25, 22, character.totalBulk, mf_15_centered);

// Wealth
mapper.textBox('credits', fileName, 1, 405, 729, 41, 26, character.credits || 0, mf_15_centered);
mapper.textBox('upb', fileName, 1, 499, 728, 86, 38, character.upb || 0, mf_8_multiline);
...
character.flatItems(character.valuables).forEach((item) => {
    if (y <= 707) {
        ...
        mapper.textBox(ref, fileName, 1, 531, y, 29, 10, item.price.join(' '), mf_8_centered);
        mapper.textBox(ref, fileName, 1, 561, y, 24, 10, item.bulk, mf_8_centered);
        y = y + 10;
    }
});
...
mapper.textBox('ethnicity', fileName, 2, 218, 49, 59, 15, character.details.ethnicity, mf_10);
mapper.textBox('nationality', fileName, 2, 283, 49, 59, 15, character.details.nationality, mf_10);
mapper.textBox('birthplace', fileName, 2, 348, 58, 39, 15, character.details.biography.birthplace, mf_10);
mapper.textBox('age', fileName, 2, 413, 49, 22, 15, character.details.age, mf_10);
mapper.textBox('gender & pronouns', fileName, 2, 440, 49, 67, 15, character.details.gender, mf_10);
mapper.textBox('height', fileName, 2, 512, 49, 31, 15, character.details.height, mf_10);
mapper.textBox('weight', fileName, 2, 549, 49, 33, 15, character.details.weight, mf_10);

mapper.textBox('appearance', fileName, 2, 218, 80, 364, 15, character.details.biography.appearance, mf_10);

// Personality
mapper.textBox('attitude', fileName, 2, 218, 124, 178, 15, character.details.biography.attitude, mf_10);
mapper.textBox('deity or philosofy', fileName, 2, 404, 124, 178, 15, character.details.deity, mf_10);
const actorEdicts = character.details.biography.edicts;
mapper.textBox('edicts', fileName, 2, 218, 142, 178, 70, ' '.repeat(10) + actorEdicts, mf_10_multiline);

const actorAnathema = character.details.biography.anathema;
mapper.textBox('edicts', fileName, 2, 404, 142, 178, 70, ' '.repeat(15) + actorAnathema, mf_10_multiline);

mapper.textBox('likes', fileName, 2, 218, 227, 364, 15, character.details.biography.likes, mf_10);
const actorDislikes = character.details.biography.dislikes;
mapper.textBox('dislikes', fileName, 2, 218, 258, 364, 15, actorDislikes, mf_10);
const actorCatchPhrases = character.details.biography.catchphrases;
mapper.textBox('catchphrases', fileName, 2, 218, 290, 364, 15, actorCatchPhrases, mf_10);

// /* Campaign notes Section */
const actorNotes = character.details.biography.campaignNotes;
mapper.textBox('campaignNotes', fileName, 2, 30, 330, 270, 96, actorNotes, mf_8_multiline);
// allies and enemies are not really good at
const actorAllies = character.details.biography.allies;
mapper.textBox('allies', fileName, 2, 312, 330, 270, 22, actorAllies, mf_6_multiline);
const actorEnemies = character.details.biography?.enemies;
mapper.textBox('enemies', fileName, 2, 312, 367, 270, 22, actorEnemies, mf_6_multiline);
const actorOrganizations = character.details.biography.organizations;
mapper.textBox('organizations', fileName, 2, 312, 404, 270, 22, actorOrganizations, mf_8_multiline);

// Actions and activities
const actorActionsActivities_y = [455, 535, 616, 695];
const actorFreeActionsActivities_y = [455, 535, 616, 695];
...
mapper.textBox(ref, fileName, 3, 63, 45, 10, 10, sf2eHelper.evalCheckMark(hasArcaneTradition), mf_10_centered);
mapper.textBox(ref, fileName, 3, 75, 45, 10, 10, sf2eHelper.evalCheckMark(hasOccultTradition), mf_10_centered);
mapper.textBox(ref, fileName, 3, 63, 57, 10, 10, sf2eHelper.evalCheckMark(hasPrimalTradition), mf_10_centered_top);
mapper.textBox(ref, fileName, 3, 75, 57, 10, 10, sf2eHelper.evalCheckMark(hasDivineTradition), mf_10_centered_top);
...
mapper.textBox(ref, fileName, 3, 129, 48, 5, 5, sf2eHelper.evalCheckMark(isPreparedCaster), mf_8_centered);
mapper.textBox(ref, fileName, 3, 129, 58, 5, 5, sf2eHelper.evalCheckMark(isSpontaneousCaster), mf_8_centered);
...
if (typeof spellProficiency !== 'undefined') {
    ...
    mapper.textBox(ref, fileName, 3, 70, 101, 18, 12, spellProficiency.attack.attributeModifier, mf_12_centered);
    mapper.textBox(ref, fileName, 3, 88, 101, 18, 12, spellProficiency.attack.proficiencyModifier, mf_12_centered);
    const spellAttackRank = spellProficiency.attack.rank;
    mapper.textBox(ref, fileName, 3, 57, 100, 5, 5, sf2eHelper.evalCheckMark(spellAttackRank >= 1), mf_8_centered);
    mapper.textBox(ref, fileName, 3, 57, 105, 5, 5, sf2eHelper.evalCheckMark(spellAttackRank >= 2), mf_8_centered);
    mapper.textBox(ref, fileName, 3, 57, 111, 5, 5, sf2eHelper.evalCheckMark(spellAttackRank >= 3), mf_8_centered);
    mapper.textBox(ref, fileName, 3, 57, 116, 5, 5, sf2eHelper.evalCheckMark(spellAttackRank >= 4), mf_8_centered);
    ...
    mapper.textBox(ref, fileName, 3, 142, 100, 5, 5, sf2eHelper.evalCheckMark(spellSaveRank >= 1), mf_8_centered);
    mapper.textBox(ref, fileName, 3, 142, 105, 5, 5, sf2eHelper.evalCheckMark(spellSaveRank >= 2), mf_8_centered);
    mapper.textBox(ref, fileName, 3, 142, 111, 5, 5, sf2eHelper.evalCheckMark(spellSaveRank >= 3), mf_8_centered);
    mapper.textBox(ref, fileName, 3, 142, 116, 5, 5, sf2eHelper.evalCheckMark(spellSaveRank >= 4), mf_8_centered);
}
// cantrips
// cantrips per day
...
// 60, 91 - 18x12
// -10, -10
mapper.textBox('cantrips per day', fileName, 3, 90, 144, 32, 20, cantripSlots, mf_12_centered);

// cantrip rank
if (cantripSlots !== '') {
    mapper.textBox('cantrip rank', fileName, 3, 184, 144, 23, 20, character.cantripRank, mf_12_centered);
}
// Cantrips
let cantripsY = 177;
const cantripsMaxY = 455;
ref = 'cantrips';
let sceType;
...
character.knownSpells
    ...
    .forEach((cantrip) => {
        ...
        if (typeof y !== 'undefined') {
            ...
            mapper.textBox(ref, fileName, 3, 30, y, 134, 10, cantrip.name, mf_8);
            mapper.textBox(ref, fileName, 3, 164, y, 26, 10, cantrip.glyph, action_8_centered);
            mapper.textBox(ref, fileName, 3, 191, y, 18, 10, 'O'.repeat(cantrip.prepared), mf_8_centered);
        }
    });
...
mapper.textBox(ref, fileName, 3, 82, 473, 9, 9, sf2eHelper.evalCheckMark(focusPointsMax >= 1), mf_8_centered);
mapper.textBox(ref, fileName, 3, 95, 473, 9, 9, sf2eHelper.evalCheckMark(focusPointsMax >= 2), mf_8_centered);
mapper.textBox(ref, fileName, 3, 108, 473, 9, 9, sf2eHelper.evalCheckMark(focusPointsMax >= 3), mf_8_centered);
...
character.knownSpells
    ...
    .forEach((spell) => {
        ...
        if (typeof y !== 'undefined') {
            ...
            mapper.textBox(ref, fileName, 3, 30, y, 169, 10, spell.name, mf_8);
            mapper.textBox(ref, fileName, 3, 180, y, 29, 10, spell.glyph, action_8_centered);
        }
    });

// Innate spells
let innateSpellsY = 674;
const innateSpellsMaxY = 766;
ref = 'innate spell';
sceType = undefined;
    ...
    .forEach((spell) => {
        ...
        if (typeof y !== 'undefined') {
            if (sceInnateSpellCount > 1 && sceType !== spell.type) {
                ...
                mapper.textBox(ref, fileName, 3, 30, y, 119, 12, spell.type, mf_10);
                sceType = spell.type;
                y = innateSpellsY;
                innateSpellsY = innateSpellsY + 10;
                endingWhiteSpace = true;
            }
            mapper.textBox(ref, fileName, 3, 30, y, 119, 10, spell.name, mf_8);
            mapper.textBox(ref, fileName, 3, 150, y, 34, 10, spell.glyph, action_8_centered);
