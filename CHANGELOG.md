...
- dnd5e support saves for dnd5e v3.3.1
- dnd5e: return an empty string when there is no background
- pf2e: return an empty string when there is no description
...
- status modifiers were applied even if they were not enabled (thank you Adam)

## [2025.3.3] 2025-03-10

### Added

- `pc` is now a supported actor type, as it is required for the **Black Flag Roleplaying** game system (thank you blueflash41)
...
- Fix NPC strike undefined sneak attack damage ([PR](https://github.com/bushvin/actor-export/pull/16))
